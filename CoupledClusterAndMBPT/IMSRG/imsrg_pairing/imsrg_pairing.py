...
def construct_basis_2B(holes, particles):
  """
   From given holes and particles list 
   numbering all possible two-particle combinations
    
   basis[n] = (p,q) 
   thus, len(basis)= (number of holes+number of particles)^2
  """  
  basis = []
  for i in holes:
    for j in holes:
...
def construct_basis_ph2B(holes, particles):
  """
   I don't see any difference with basis_2B ?? 
  """  
  basis = []
  for i in holes:
    for j in holes:
...
def construct_index_2B(bas2B):
  """
    look up table of 2-body index
    index[(i,j)]= n <-> basis_2B[n]=(i,j) 
  """  
  index = { }
...
def ph_transform_2B(Gamma, bas2B, idx2B, basph2B, idxph2B):
  """
  convert Gamma to Gamma_ph 
  
  Gamma_ph[a,bp,c,dp] = - Gamma[a,d,c,b]
...
def inverse_ph_transform_2B(Gamma_ph, bas2B, idx2B, basph2B, idxph2B):
  """ 
  reverse Gamma_ph to Gamma 
  
  Gamma[a,b,c,d] = - Gamma_ph[a,dp,c,bp]
...
#
# H = E+  sum f*N[1b-op]+0.25*sum Gamma* N(2b-op)   
#-----------------------------------------------------------------------------------
def calc_fod_norm(f, user_data):
  """ 
   off-diagonal elements norm 
   < 1p1h|H|0> 
   <0|H|1p1h>
  """  
...
# in 2B basis index. N[ij,ij]=n_i -n_j 
...
def eta_brillouin(f, Gamma, user_data):
  """ 
   brillouin generator choice. 
   simply use G=1 in (10.129) and (10.130)
   
   eta_{ph} = f_{ph} 
   eta_{pp',hh'} = Gamma_{pp',hh'} 
   
   eta_{hp} = -f_{ph} 
   eta_{hh',pp'}= -Gamma_{pp',hh'}
  """  
...
def eta_imtime(f, Gamma, user_data):
  ...
  for a in particles:
    for i in holes:
      dE = f[a,a] - f[i,i] + Gamma[idx2B[(a,i)], idx2B[(a,i)]] # eq.(10.145)
...
def eta_white(f, Gamma, user_data):
  ...
  for a in particles:
    for i in holes:
      denom = f[a,a] - f[i,i] + Gamma[idx2B[(a,i)], idx2B[(a,i)]] #eq.(10.145)
      val = f[a,i]/denom
      eta1B[a, i] =  val
      eta1B[i, a] = -val 
...
def eta_white_mp(f, Gamma, user_data):
  """ 
   eta1B is the same as eq.(10.141)
   But, eta2B is using eq.(10.138)
   without Gamma terms. 
  """  
  dim1B     = user_data["dim1B"]
  particles = user_data["particles"]
...
def eta_white_atan(f, Gamma, user_data):
  """ 
  White generator modified in eq. (10.143)
  """  
  dim1B     = user_data["dim1B"]
  particles = user_data["particles"]
  holes     = user_data["holes"]
...
def eta_wegner(f, Gamma, user_data):
  """
   Wegner generator in eq. (10.148) and (10.149)
  """
  dim1B     = user_data["dim1B"]
  holes     = user_data["holes"]
  particles = user_data["particles"]
  ...
  # diagonal and off-diagonal 1B Hamiltonian 
  for a in particles:
    for i in holes:
      fod[a, i] = f[a,i]
      fod[i, a] = f[i,a]
  ...
  # diagonal and off-diagonal 2B Hamiltonian 
  for a in particles:
    for b in particles:
      for i in holes:
        for j in holes:
...
def eta_wegner2(f, Gamma, user_data):
  """
   Wegner generator in eq. (10.148) and (10.149)
...
def flow_imsrg2(eta1B, eta2B, f, Gamma, user_data):
  ...
  # eq.(10.104). Note that this equation already used anti-symmetry of generator, potential
  dE = 0.0
  ...
  #############################        
  # one-body flow equation  eq.(10.105)
  df  = np.zeros_like(f)
...
def derivative_wrapper(t, y, user_data):
  ...
  occC_2B   = user_data["occC_2B"]
  occphA_2B = user_data["occphA_2B"]
  calc_eta  = user_data["calc_eta"] # generator function choice 
  calc_rhs  = user_data["calc_rhs"] # derivative function choice 
...
def main(delta,g,
         dim1B=8,holes =[0,1,2,3],particles=[4,5,6,7],
         generator_choice= eta_white ):
  # grab delta and g from the command line
  #delta      = float(argv[1])
  #g          = float(argv[2])

  particles  = 4 # <--seems to be redundant 
  ...
  user_data  = {
    ...
    "calc_eta":   generator_choice, #eta_white,  # specify the generator (function object)
