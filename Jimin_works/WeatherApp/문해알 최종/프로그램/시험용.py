...
def main_ui():
    ...
    def setting():
        #세팅 값. 뭘 넣을진 모르겠지만 일단 만들어봐요
        setting_window = tk.Toplevel(root)
        setting_window.title("설정")
        setting_window.geometry("500x400")

        tk.Label(setting_window, text="뭘넣어야할까요? 아님 그냥 아이콘만 넣고 설명안해도 괜찮을듯한데..", font=("Arial", 12)).pack(pady=10)
    ...
    def open_more():
        ...
        def show_color_recommendations():
            ...
            tk.Label(color_window, text="- 위아래 깔맞춤은 피하는 것이 좋습니다.\n"
                                         "- 자세한 것은 아래 이미지를 참고하세요.",
                     #이미지 넣기
    ...
    setting_button = tk.Button(root, text="설정", command=setting, font=("Arial", 12))
