    ...
    tolerance(1e-8), convergence_ediff(7,0), convergence_EHF(7,0), freeze_occupations(true)
{
   ...
   C             = arma::mat(norbits,norbits,arma::fill::eye); //for transform matrix C
   Vij           = arma::mat(norbits,norbits,arma::fill::zeros); //for monopole (V_2)_{ij}
   V3ij          = arma::mat(norbits,norbits,arma::fill::zeros); //for monopole (V_3)_{ij}
   F             = arma::mat(norbits,norbits);      //for Fock matrix   
   ...
   BuildMonopoleV(); // setup Vij 
   if (hbare.GetParticleRank()>2)
   {
      BuildMonopoleV3(); //setup V3ij 
   }
   UpdateDensityMatrix(); // make density matrix from C
   UpdateF(); // actually make F from density matrix,KE,Vij, V3ij   
...
void HartreeFock::FillLowestOrbits()
{
  ...
  arma::mat F_trans = C.t() * F * C; // F in HF basis 
//  arma::uvec sorted_indices = arma::stable_sort_index( F.diag() );
  ...
  for (auto i : sorted_indices)
  {
    ...
    Orbit& oi = modelspace->GetOrbit(i);
    // if oi is proton orbit and there are remaining protons
    // then set the orbit is a hole orbit 
    // and set occupation number as 1 
    //     or fractional occupation number (remaining proton)/(2j+1) 
