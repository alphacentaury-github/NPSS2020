...
/// collect kets from model space 
/// for two-body channel { |pq>_{JPT} } 
/// 
/// 1. J = N%(tbjmax+1)  # it is not clear how N is defined..
///
/// 2. parity = (N/(tbjmax+1))%2;
/// 
/// 3. Tz = (N/(2*(tbjmax+1))-1)
/// 
/// 4. make KetMap from model space Kets 
///    by CheckChannel_ket(ket) 
///
/// 5. Get KetIndex_pp in this channel from modelspace KetIndex_pp. 
///    other index of _hh,_ph,_cc ..in a similar way.
///
/// 6. Get Ket_occ_hh and Ket_unocc_hh in this channel from model space. 
///        Ket_occ_ph     Ket_unocc_ph in a similar way 
...
/// valence space have a core and involving shells/orbits
/// orbit starting 'p' is for proton and 'n' for neutron 
...
///  
/// valence == "0hw-shell" ??? 
///            comma separated list :  interpet as a comma-separated list of core followed by valence orbits
///            "FCI" : FCI space, so no core, all orbits are valence ???
///            "" no valence space. we've got a single-reference. ???   
...
/// model space construct from file 
...
/// defines all orbits and assign cvq and occ and Aref, Zref
/// then construct all Kets from orbits.  
void ModelSpace::Init(int emax, std::map<index_t,double> hole_list, std::vector<index_t> core_list, std::vector<index_t> valence_list)
{
   ...
   // make all orbits from given emax,core_list,hole_list,valence_list   
   // set occ and cvq value from core, hole, valence list  
   norbits = (Emax+1)*(Emax+2); // why? 
   ...
   {
     Orbit& oh = GetOrbit(h);
     Aref += (oh.j2+1)*oh.occ; //count number of particle from hole occupation (occ=1 actually means 2j+1 particles)  
     if (oh.tz2 < 0) Zref += (oh.j2+1)*oh.occ; //tz2<0 means proton? strange... 
   }
   SetupKets();
}
...
/// determine A and Z from input sting like 'He4' 
...
/// change reference core,valence,hole and re-construct model space  
/// If we have no valece space, assume it's a single ref and core should equal the reference.
...
/// (0) assign index number to orbit 
/// (1) append one orbit to Orbits , particles, valence, holes, core, qspace, 
///                     proton_orbits, neutron_orbits vectors. 
/// (2) if j2 value is increased, increase Jmax and chennel numbers 
/// (3) append (l,j2,tz2) to OneBodyChannels at orbit index.  
///      i.e. OneBodyChannels[indx] shows the channel of orbit belongs. 
...
/// convert orbit string to orbit index 
...
/// (TwoBodyChannelIndex) = (t+1)*2*(TwoBodyJmax+1) + p*(TwoBodyJmax+1) + j 
// why no OneBodyChannelIndex ? In fact why need even OneBodyChannel? 
// because one orbit already have l,j,tz2 info... 
...
/// (1) From orbits, construct all kets |pq> with q>= p. ( |qp> is inferred later)  
/// (2) Each Kets is assigned KetIndex and they are divided into 
///        cc, vc, vv, qv, qq, pp, ph, hh cases. (why no cq ?)  
/// (3) assign occ_ph, occ_hh from ket corresponding n_p n_q or (1-n_p)(1-n_q)  
...
//   Kets.resize(nkets);
  ...
  {
    Ket& ket = Kets[index]; // here index is for Ket not orbit index. 
...
//    if (occp>OCC_CUT or occq>OCC_CUT)
   ...
   // TwoBodyChannels are already defined?? 
