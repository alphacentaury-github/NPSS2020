...
int main(int argc, char** argv)
{
  ...
  //test 
  cout << "!!treating input argument done" << endl;
...
//    if( not test.good() and fmt2!="oakridge_binary")
  ...
  cout << "!!reading input file test done" << endl;
  ...
  //test 
  cout << "!!set rw done" << endl;

//  ModelSpace modelspace;
  cout << "!!treat custom valence space start" << endl;//test 
  ...
  cout << "!!treat custom valence space done" << endl;//test 
  cout << " valence_space string =  " << valence_space << endl; //test 	  
  cout << "!!setup modelspace start" << endl;//test 

  ModelSpace modelspace = ( reference=="default" ? ModelSpace(eMax,valence_space) : ModelSpace(eMax,reference,valence_space) );

  cout << "!!modelspace declared " << endl;//test 
  ...
  cout << "!!modelspace nsteps=" << nsteps <<endl;//test 
  ...
  cout << "!!modelspace setup done. print model space for test" << endl;//test 

 // here let us print out the model space to undestand the details  
// //  
  cout << "!!test_out : Emax=" << modelspace.Emax << endl; 
  cout << "!!test_out : E2max=" << modelspace.E2max << endl; 
  cout << "!!test_out : Lmax2=" << modelspace.Lmax2 << endl; 
  cout << "!!test_out : norbits=" << modelspace.norbits << endl; 
  cout << "!!test_out : hbar_omega=" << modelspace.hbar_omega << endl; 
  cout << "!!test_out : target_mass=" << modelspace.target_mass << endl; 
  cout << "!!test_out : target_Z=" << modelspace.target_Z << endl; 
  cout << "!!test_out : Aref=" << modelspace.Aref << endl; 
  cout << "!!test_out : Zref=" << modelspace.Zref << endl; 
  cout << "!!test_out : vector holes" << endl; 
  for (auto i = modelspace.holes.begin(); i != modelspace.holes.end(); ++i)
    std::cout << *i << ' ';
  cout << endl;
  cout << "!!test_out : vector particles" << endl; 
  for (auto i = modelspace.particles.begin(); i != modelspace.particles.end(); ++i)
    std::cout << *i << ' ';
  cout << endl;
  cout << "!!test_out : vector valence" << endl; 
  for (auto i = modelspace.valence.begin(); i != modelspace.valence.end(); ++i)
    std::cout << *i << ' ';
  cout << endl;
  cout << "!!test_out : vector qspace" << endl; 
  for (auto i = modelspace.qspace.begin(); i != modelspace.qspace.end(); ++i)
    std::cout << *i << ' ';
  cout << endl;
  cout << "!!test_out : vector core" << endl; 
  for (auto i = modelspace.core.begin(); i != modelspace.core.end(); ++i)
    std::cout << *i << ' ';
  cout << endl;
//  cout << "!!test_out : proton_orbits=" << modelspace.proton_orbits << endl; 
//  cout << "!!test_out : neutron_orbits=" << modelspace.neutron_orbits << endl; 
//  cout << "!!test_out : KetIndex_pp=" << modelspace.KetIndex_pp << endl; 
//  cout << "!!test_out : KetIndex_ph=" << modelspace.KetIndex_ph << endl; 
//  cout << "!!test_out : KetIndex_cc=" << modelspace.KetIndex_cc << endl; 
//  cout << "!!test_out : KetIndex_vc=" << modelspace.KetIndex_vc << endl; 
//  cout << "!!test_out : Ket_unocc_hh=" << modelspace.Ket_unocc_hh << endl; 
//  cout << "!!test_out : Ket_occ_hh=" << modelspace.Ket_occ_hh << endl; 
  cout << "!!test_out : OneBodyJmax=" << modelspace.OneBodyJmax << endl; 
  cout << "!!test_out : TwoBodyJmax=" << modelspace.TwoBodyJmax << endl; 
//  cout << "!!test_out : OneBodyChannels=" << modelspace.OneBodyChannels << endl; 
//  cout << "!!test_out : OneBodyChannels=" << modelspace.TwoBodyChannels << endl; 
//  cout << "!!test_out : SortedTwoBodyChannels=" << modelspace.SortedTwoBodyChannels << endl; 
  cout << "!!test_out : Orbits can be found in HF results " << endl; 
    for (auto i = 0;i < modelspace.norbits; ++i)
    {
       Orbit oi = modelspace.GetOrbit(i) ;	    
       cout << oi.index << " "<< oi.n << " " << oi.l << " "<< oi.j2 <<" " << oi.tz2;
       cout << endl;
    }    
  cout << "!!test_out : modelspace Kets " << endl;
  //typedef unsigned long long int index_t; 	  
  for (index_t index=0; index < modelspace.Kets.size() ;++index)
  { 
	cout << index << " p="<< modelspace.Kets[index].p << " q=" << modelspace.Kets[index].q;
        cout << " cvq12=" << modelspace.Kets[index].op->cvq << modelspace.Kets[index].oq->cvq <<  endl; 	
  }  
 ...
 ///
  ...
  if (inputtbme != "none")
  {
    cout << "reading TBME" << inputtbme << " " << fmt2 << endl; //test 
    if (fmt2 == "me2j")
  ...
  // navratil format there is no one-body operator and actual TBME is 
  //
  // TBME_nav = <ab |Trel +V_2|cd > if doCoM_corr
  //            < ab|V_2|cd> if not doCoM_corr (default) 
  //
  // Hbare=  (1-1/A)*T_1 + (T_2+V_2) =Trel + V_2
  // and have one-body operator. 
  // Trel = T_1 - T_{CM} = (1-1/A)*T_1 + T_2 
  //  
  ...
  cout << "!!Hbare preparation is done" << endl; //test 
  // All Hbare is prepared. Hbare operator has
  // one-body operator and two-body operator 
  // each holds matrix elements
  // OneBody(i,j) as matrix , (i,j=sp index) 
  // TwoBody(ch_bra, ch_ket)(i_bra,j_ket)  as TwoBodyME
  //           ch_bra and ch_ket is channel index
  //           i_bra and j_ket is a ket-index 
...
//  cout << "EHF = " << hf.EHF << endl;
  ...
  cout << "!!Creating HNO" << endl; //test
//  Operator HNO;
//  Normal ordering w.r.t. what? 
//  w.r.t. core ? or reference ? 
  Operator& HNO = Hbare;
  if (basis == "HF" and method !="HF")
    HNO = hf.GetNormalOrderedH(); //normal ordering from hf ? 
  else if (basis == "oscillator")
    HNO = Hbare.DoNormalOrdering(); //normal ordering from Hbare 
...
//  }
  ...
  for (auto& op : ops)
  {
     if (basis == "HF") op = hf.TransformToHFBasis(op); //transform operators into HF basis 
...
//  hf.PrintSPE();
  ...
  // If method is "HF" or "MP3", 
  // end program here. 
  // output is HF orbits energies
  //           MBPT results 
  //           and normal ordered Hamiltonian   
  ...
  // If method is "FCI",
  // write the Hamiltonian in nushellx format 
  // for shell model calculation later 
  // and end program. 
  // i.e. "FCI" means to obtain shell model interaction
  // in full model space without IMSRG transformation. 
  ...
  // Now normal ordered Hamiltonian in HF basis with HF reference 
  // and Operators are prepared.
  // One can start IMSRG flow. 
...
//  imsrgsolver.SetHin(Hbare);
  ...
  imsrgsolver.Solve(); // Actually solve IMSRG equation 
...
//      rw.WriteOperatorHuman(ops[i],intfile+opnames[i]+"_step2.op");
  ...
  {
    renormal_order = modelspace.holes.size() != modelspace.core.size(); //if core is not the same as reference 
  ...
  // Once IMSRG is solved w.r.t. reference which gives holes
  // One have normal Ordered Hamiltonian for the reference.
  // But, if we want VS-IMSRG, one undo the normal ordering 
  // HNO -> Hbare_new 
  // Then, redo normal ordering of Hbare_new w.r.t.  core. 
  // Hbare_new -> HNO_core 
  // Now HNO_core gives the effective interactions 
  // above the core. 
  // This can be used for shell model calculation. 
  if ( renormal_order )
  {
