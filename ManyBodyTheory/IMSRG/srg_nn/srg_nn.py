...
# --> in this example, it is a uniform momentum mesh 
#------------------------------------------------------------------------------
...
def plot_snapshots(Hs, flowparams, momenta, qMax):
  ...
  for s in range(Hs.shape[0]):
    ...
    img = grid[s].imshow(h,
                          ...
                          # filterrad=10,
                          norm=SymLogNorm(linthresh=0.0001, vmax=2., vmin=-2.0,base=10),   # normalize 
...
#
# d V(q_i,q_j;s)/ds ==> d y(m;s)/ds as y(m) = V(q_i,q_j) as 1-d array   
#------------------------------------------------------------------------------
...
def main():
  ...
  solver = ode(derivative,jac=None) # this sets up the flow equation

  # equations may get stiff, so we use VODE and Backward Differentiation
  solver.set_integrator('vode', method='bdf', order=5, nsteps=1000)
  solver.set_f_params(T) # third parameter of derivative(lam,y,T)
  ...
  return T,Vs,conversion_matrix 

#------------------------------------------------------------------------------
# make executable
#------------------------------------------------------------------------------
if __name__ == "__main__": 
  T,Vs,conversion_matrix = main()