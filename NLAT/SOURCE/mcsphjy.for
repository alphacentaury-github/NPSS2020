...
C        DIMENSION CSJ(0:250),CDJ(0:250),CSY(0:250),CDY(0:250)
C        WRITE(*,*)'Please enter n,x,y (z=x+iy) '
C        READ(*,*)N,X,Y
C        WRITE(*,30)N,X,Y
C        Z=CMPLX(X,Y)
C        IF (N.LE.8) THEN 
...
C           READ(*,*)NS
C        ENDIF
C        CALL CSPHJY(N,Z,NM,CSJ,CDJ,CSY,CDY)
C        WRITE(*,*)
C        WRITE(*,*)'  n      Re[jn(z)]        Im[jn(z)]',
C     &  '        Re[jn''(z)]       Im[jn''(z)]'
C        WRITE(*,*)'--------------------------------------------',
C     &  '----------------------------'
C        DO 10 K=0,NM,NS
C10         WRITE(*,20)K,CSJ(K),CDJ(K)
C        WRITE(*,*)
C        WRITE(*,*)'  n      Re[yn(z)]        Im[yn(z)]',
C     &  '        Re[yn''(z)]       Im[yn''(z)]'
C        WRITE(*,*)'--------------------------------------------',
C     &  '----------------------------'
C        DO 15 K=0,NM,NS
C15         WRITE(*,20)K,CSY(K),CDY(K)
C20      FORMAT(1X,I3,4D17.8)
C30      FORMAT(3X,6HNmaz =,I3,',     ','z = ',F8.1,'+ i',F8.1)
C        END


C        SUBROUTINE CSPHJY(N,Z,NM,CSJ,CDJ,CSY,CDY)
        SUBROUTINE CSPHJY(N,Z,NM,CSJ)
C
C       ==========================================================
