...
C		    
      ...
      READ(5,15) ELABI,XMES,THMIN,THMAX,THINC
      READ(5,11) LMAX,NXMX
...
C      
C======================================================================
      CALL OPTMDL(KTRL,KOUT,LMAX,NXMX,NOTH,
...
CCCCCC
C======================================================================
      SUBROUTINE OPTMDL(KTRL,KOUT,LMAX,NXMAX,NAMAX,TMST,PMST,TZ,PZ,
     1           ELABI,XMES,THETAD,VO,WO,WS1,WS2,AO,AI,AIS1,AIS2,
     2           RO,RI,RIS1,RIS2,RC,VSP,ASP,RSP)
C======================================================================
C
...
C
  ...
  300 CONTINUE
     ...
     1               CRRUTH,CREL,CRATIO)
      ...
      IF (KTRL4.NE.0) THEN
	 CALL ELETC(LMAX,WN,ETA,SMAT,FUST,
     1              FCROS,TCROS)
      ENDIF
...
CCCCCC
C======================================================================
      SUBROUTINE ELETC(LMAX,WN,ETA,SMAT,FUST,
     O                  FCROS,RECROSS)
C======================================================================
C
...
C      
      ...
      DO 100 L1=2,LMAX
         ...
         JDN=2*L
         FL0=DFLOAT(L)
         FCROS=FCROS+(FUST(JUP)+FUST(JDN))
         FCRO1=FCRO1+(FUST(JUP)+FUST(JDN))*FL0
         FCRO2=FCRO2+(FUST(JUP)+FUST(JDN))*FL0**2
  100 CONTINUE
c         AVEL1=FCRO1/FCROS
c         AVEL2=FCRO2/FCROS
c         AVEL2=DSQRT(AVEL2)
C
      ...
      DO 200 L1=1,LMAX
         ...
         IF (L1.EQ.1) THEN
            ...
            TCOE(JUP)  =1.D0-SMAT(JUP)*CONJG(SMAT(JUP))
            RECROS(JUP)=10.D0*TCOE(JUP)*FJUP*AREA*0.5D0
            RECROSS=RECROSS+(RECROS(JUP))
         ELSE
         CL(JUP)    =(SMAT(JUP)-1.D0)/(2.D0*TTI)
         TCOE(JUP)  =1.D0-SMAT(JUP)*CONJG(SMAT(JUP))
         RECROS(JUP)=10.D0*TCOE(JUP)*FJUP*AREA*0.5D0
         CL(JDN)    =(SMAT(JDN)-1.D0)/(2.D0*TTI)
         TCOE(JDN)  =1.D0-SMAT(JDN)*CONJG(SMAT(JDN))
         RECROS(JDN)=10.D0*TCOE(JDN)*FJDN*AREA*0.5D0
         RECROSS=RECROSS+(RECROS(JUP)+RECROS(JDN))
         ENDIF
         IF (ETA.EQ.0.0D0) THEN
            IF (L1.EQ.1) THEN
                SMM1=1.0D0-SMAT(JUP)
                SMM12=SMM1*CONJG(SMM1)*FJUP*0.5D0
                TCROS(JUP)=10.0D0*(1.0D0-REAL(SMAT(JUP)))*FJUP*AREA
                TOCROSS=TOCROSS+TCROS(JUP)
            ...
            TCROS(JUP)=10.0D0*(1.0-REAL(SMAT(JUP)))*FJUP*AREA
            TCROS(JDN)=10.0D0*(1.0-REAL(SMAT(JDN)))*FJDN*AREA
            TOCROSS=TOCROSS+TCROS(JUP)+TCROS(JDN)
  ...
  400 CONTINUE
      ...
      DO 500 L=1,JMAX
         ...
         IF (ETA.EQ.0.0D0) THEN
            WRITE(IW,910) L1,J2,ABSS(L),PHAS(L),TCOE(L),
     1                    FUST(L),RECROS(L),TCROS(L),L1,J2
         ELSE
         WRITE(IW,912) L1,J2,ABSS(L),PHAS(L),TCOE(L),
     1                 FUST(L),RECROS(L),L1,J2
         ENDIF
  500 CONTINUE
      IF (ETA.EQ.0.0D0) THEN
         WRITE(IW,920) FCROS,RECROSS,TOCROSS
...
c      WRITE(IW,970) SAR
  ...
  900 FORMAT(//,1H ,'PARTIAL WAVE INFORMATION'//
     ...
     3          ,'   L  J2'/21X,'(RAD)',17X,'(MB)',6X,'(MB)'/)
  902 FORMAT(//,1H ,'PARTIAL WAVE INFORMATION'//
     1          1H ,'  L  J2',' ABS S-MAT',' PHA S-MAT',
     2              '   T-COEFF',4X,'FUSION','  REACTION'
     3          ,'   L  J2'/21X,'(RAD)',17X,'(MB)',6X,'(MB)'/)
